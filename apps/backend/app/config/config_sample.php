 ...
 */
// Read database configuration from environment variables (Docker-friendly)
$pg_host = getenv('POSTGRES_HOST') ?: 'postgres';
$pg_port = getenv('POSTGRES_PORT') ?: '5432';
$pg_db   = getenv('POSTGRES_DB') ?: 'lemalagasy_db';
$pg_user = getenv('POSTGRES_USER') ?: 'lemalagasy_user';
$pg_pass = getenv('POSTGRES_PASSWORD') ?: 'lemalagasy_password';

// Allow override for driver if needed (default: pgsql)
$db_driver = getenv('DB_DRIVER') ?: 'pgsql';

return [
	'database' => [
		// Driver: 'pgsql' (Postgres) or 'mysql' etc.
		'driver' => $db_driver,
		'host' => $pg_host,
		'port' => $pg_port,
		'dbname' => $pg_db,
		'user' => $pg_user,
		'password' => $pg_pass,
		// Optional PDO DSN if your bootstrap expects it:
		// 'dsn' => sprintf('%s:host=%s;port=%s;dbname=%s', $db_driver, $pg_host, $pg_port, $pg_db),

		// If you prefer sqlite for quick tests, uncomment and set file_path
